...
import FlagToggle from "../../Common/FlagToggle";   // ✅ ADDED
...
const ProductsTab = ({ onSwitchTab }) => {
  ...
  // ── normalizedProducts: ensure every product has a tags array ✅ ADDED ─────
  const normalizedProducts = useMemo(
    () => products.map((p) => ({ ...p, tags: p.tags || [] })),
    [products]
  );
  ...
  // ── Indeterminate state for flag toggles ✅ ADDED ─────────────────────────
  useEffect(() => {
  ...
  // ── Bulk flag update handler ✅ ADDED ─────────────────────────────────────
  const handleBulkFlagUpdate = async (flagType) => {
    ...
    if (!stateEntry) { console.error("Invalid flagType:", flagType); return; }

    const [currentValue, setValue] = stateEntry;
    const isIndeterminate = flagType === "today-arrival" ? todayIndeterminate : saleIndeterminate;
    ...
    // Optimistic update
    setValue(newValue);
    dispatch(optimisticBulkTagUpdate({ slugs, flagType, value: newValue }));
  ...
  const filteredProducts = useMemo(() => {
    return normalizedProducts.filter((product) => {
      ...
      if (range) {
        ...
        if (raw) { const d = new Date(raw); matchesDate = d >= range.start && d <= range.end; }
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
        {someSelected ? (
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {/* ✅ Flag Toggles — Today Arrival & On Sale */}
